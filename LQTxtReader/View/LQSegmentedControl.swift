...
@objc protocol LQSegmentedControlDelegate:NSObjectProtocol {
    
    /// 点击按钮
    @objc optional func segmentedControl(segmentedControl:LQSegmentedControl, clickButton button:UIButton, index:NSInteger)
}

class LQSegmentedControl: UIView {

    /// 代理
    weak var delegate:LQSegmentedControlDelegate?
    ...
    init(_ delegate:LQSegmentedControlDelegate? = nil) {
    ...
    private func addSubviews() {
        ...
        for i in 0..<count {
            ...
            button.addTarget(self, action: #selector(LQSegmentedControl.clickButton(button:)), for: .touchDown)
