...
/// 电池宽推荐使用宽高
let LQBatterySize:CGSize = CGSize(width: 25, height: 10)

/// 电池量宽度 跟图片的比例
private var HJBatteryLevelViewW:CGFloat = 20
private var HJBatteryLevelViewScale = HJBatteryLevelViewW / LQBatterySize.width

class LQBatteryView: UIImageView {
    ...
    /// 初始化
    convenience init() {
        self.init(frame: CGRect(x: 0, y: 0, width: LQBatterySize.width, height: LQBatterySize.height))
    }
    
    /// 初始化
    override init(frame: CGRect) {
        super.init(frame: CGRect(x: 0, y: 0, width: LQBatterySize.width, height: LQBatterySize.height))
        addSubviews()
    }
    ...
    override func layoutSubviews() {
        ...
        let spaceW:CGFloat = 1 * (frame.width / LQBatterySize.width) * HJBatteryLevelViewScale
        let spaceH:CGFloat = 1 * (frame.height / LQBatterySize.height) * HJBatteryLevelViewScale
